...
\begin{itemize}
    ...
    \item $\int 1/\sqrt{1-x^2} dx = sin^-1(x)$
    \item $\int -1/\sqrt{1-x^2} dx = cos^-1(x) = arcos(x)$
    \item $\int sin(x) dx = -cos(x), \int cos(x) dx = sin(x)$
    \item $\int a^x dx = a^x/ln(a)$
    \item $\int ln(x) dx = xln(x) - x$
\end{itemize}

\subsection{function of a RV}

Given discrete RV $X$, the PMF of $Y=g(X)$ is

\begin{equation}
    P_Y(y)=\sum_{x:g(x)=y} P_X(x)
\end{equation}

Given $Y=aX$, where $a>0$, $Y$ has CDF and PDF,
...
\begin{equation}
    E[X] = \mu_X = \sum_{x \in S_X} x P_X(x) = \int_{-\infty}^{\infty} x f_X(x) dx
\end{equation}
...
\begin{equation}
    Var[X] = E[X^2] - \mu_X^2 = \int_{-\infty}^{\infty} x^2 f_X(x) dx + \mu_X^2
\end{equation}
...
Given relationship of 2 RVs as $Y = g(X)$,

\begin{equation}
    Var[Y] = \mu_Y = \sum_{x \in S_X} g(x) P_X(x) =  \int_{-\infty}^{\infty} g(x) f_X(x) dx
\end{equation}
...
in which $0 < p < 1$, and $E[X] = p$, $Var[X] = p(1-p)$

\subsection{Binomial}

Binomial $(n, p)$ RV $X$ has PMF,

\begin{equation}
    P_X(x) = \binom{n}{x} p^x (1-p)^{n-x}
\end{equation}

in which $0 < p < 1$, and $n \geq 1$, $E[X] = np, Var[X] = np(1-p)$

\subsection{exponential}

Continuous exponential ($\lambda$) RV $X$ has PDF,
...
with $\lambda > 0$, and $E[X] = 1/ \lambda$, $Var[X] = 1/ \lambda ^2$
